...
* **共有ライブラリ方式**: DB 操作・プロンプト・tmux 操作は `agmsg-common-plugin` に委譲。`agmsg-qwencode-plugin` と共通コードを共有し、一貫したセキュリティ境界（team/agent フィルタ必須）を強制。
...
本プラグインは `repo/agmsg-common-plugin` に依存します。プラグイン配置時に `../agmsg-common-plugin/` が相対パスで参照可能である必要があります。

```bash
# repo/ ディレクトリ構成（推奨）
repo/
├── agmsg-common-plugin/        # 共有ライブラリ
├── agmsg-opencode-plugin/      # 本プラグイン
└── agmsg-qwencode-plugin/      # 別エージェント用
```
...
## v2 → v3 変更履歴 (2026-06-11)

| 項目 | v2 | v3 |
|------|----|----|
| **DB 操作** | インライン prepared statement ×6 | `agmsg-common-plugin` に委譲 |
| **プロンプト** | インライン `formatMessage()` | `NOTIFICATION()` に委譲 |
| **ツール数** | 4 ツール | 変更なし（4ツール維持） |
| **コード量** | 317 行 | -60 行の削減 |
| **共有** | 独立したコード | `agmsg-qwencode-plugin` と共通ライブラリを共有 |
