...
# index.ts と common.ts を上記ディレクトリに配置
```
...
```
PluginOptions (createPlugin 引数) > 環境変数 > config.yaml > 初期設定
```

### config.yaml

`AGMSG_STORAGE_PATH/config.yaml` に設定を記録できます。**初回起動時に自動生成されます**（オンボーディング）。既存のファイルは上書きしません。

`config.yaml.example` を参考にしてください:

```yaml
team_name: "default_team"     # AGMSG_TEAM で上書き可能
agent_name: "opencode"        # AGMSG_AGENT で上書き可能
watch_interval: 30000         # AGMSG_WATCH_INTERVAL で上書き可能 (ms)
```
...
| 変数 | デフォルト値 | 説明 |
|------|-------------|------|
| `AGMSG_TEAM` | config.yaml の値 または `default_team` | メッセージルーティング対象のチーム |
| `AGMSG_AGENT` | config.yaml の値 または `opencode` | エージェント名（agmsg の `to_agent` と一致させる） |
| `AGMSG_STORAGE_PATH` | `~/.agents/skills/agmsg` | agmsg データのベースディレクトリ。**ファイル名を含まない**。内部で `{AGMSG_STORAGE_PATH}/db/messages.db` が実際の DB パスになる |
| `AGMSG_WATCH_INTERVAL` | config.yaml の値 または `30000` | アイドル検知のポーリング間隔（ミリ秒） |
